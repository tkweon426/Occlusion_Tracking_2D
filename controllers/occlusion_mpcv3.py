...
class FastOcclusionMPC_v3:
    def __init__(
        ...
        w_effort=0.1,
        w_distance=10.0,
        w_occlusion_los=50.0,    # Weight for full LoS perpendicular distance barrier (v1)
        w_occlusion_mid=50.0,    # Weight for midpoint barrier (v2)
        barrier_steepness=5.0,
        cull_radius=25.0,
        kp_yaw=4.0,
        kd_yaw=2.0,
        tau_z_max=1.0,
        ...
        self.w_occlusion_los = w_occlusion_los
        self.w_occlusion_mid = w_occlusion_mid
    ...
    def _cost_and_jac(self, u_flat, drone_xy_vel, evader_traj, nearby_obstacles):
        ...
        for obs in nearby_obstacles:
            ...
            # ---- 2a. Full LoS perpendicular distance barrier (v1) ----
            if self.w_occlusion_los > 0:
                ...
                P_los = self.w_occlusion_los * np.exp(-alpha * (d - 1.0))
                occ_cost += np.sum(P_los[mask])
                ...
                S_sign = np.sign(S)
                dd_dx = (S_sign * dS_dx * N2 - S_abs * 0.5 * dN2_dx) / (N2 * N_seg)
                dd_dy = (S_sign * dS_dy * N2 - S_abs * 0.5 * dN2_dy) / (N2 * N_seg)

                dP_los_dx = -alpha * P_los * dd_dx
                dP_los_dy = -alpha * P_los * dd_dy

                dJ_dx += np.where(mask, dP_los_dx, 0.0)
                dJ_dy += np.where(mask, dP_los_dy, 0.0)

            # ---- 2b. Midpoint barrier (v2) ----
            if self.w_occlusion_mid > 0:
                ...
                P_mid = self.w_occlusion_mid * np.exp(-alpha * (dM - 1.0))
                occ_cost += np.sum(P_mid)
                ...
                dP_mid_dx = -alpha * P_mid * ddM_dx
                dP_mid_dy = -alpha * P_mid * ddM_dy
