...
def basic_chase_controller(drone_state, evader_state):
    ...
    # --- 1. World-frame PD acceleration command ---
    dx = ex - x
    dy = ey - y

    kp = 2.0   # position gain
    kd = 2.8   # velocity damping

    ax_des = kp * dx - kd * x_dot
    ay_des = kp * dy - kd * y_dot

    # Saturate so we stay within achievable tilt angles
    a_max = 8.0  # m/s^2  (<< g, keeps angles well under 45 deg)
    a_mag = np.hypot(ax_des, ay_des)
    if a_mag > a_max:
        ax_des *= a_max / a_mag
        ay_des *= a_max / a_mag

    # --- 2. Invert kinematics: world accel → body angles ---
    ...
    # --- 3. Yaw: keep drone facing the evader (PD on heading error) ---
    target_angle = np.arctan2(dy, dx)
    yaw_error = (target_angle - psi + np.pi) % (2 * np.pi) - np.pi

    kp_yaw = 4.0   # rad/s^2 per rad  (tau_z / I_zz = 50*tau_z, so effective kp=200)
    kd_yaw = 2.0
